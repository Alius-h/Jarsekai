...
const handler = async (m, {conn, command, args, text, usedPrefix}) => {
  ...
  try {
   ...
   if (command === 'شغل') {
    ...
    } else if (command === 'سغل2') {
      additionalText = 'video 🎥';
    }
    const texto1 = `*◉—⌈🎶Elaina AI (مُنزل الموسيقى)🎶⌋—◉*
...
❏ *_جاري إرسال ${additionalText}، يرجى الانتظار لحظة..._*`.trim();
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            await conn.reply(m.chat, '*[🔥] 🔥 Elaina AI: يوجد بعض المشاكل في معالجة هذا الأمر، يرجى التحلي بالصبر. سأقوم بإرسال طلبك قريبًا*', m);
          }
        }
      }
    }
    if (command == 'شغل' || command == 'playdoc2') {
      try {
        const qu = '360';
        const q = qu + 'p';
        const v = yt_play[0].url;
    ...
    throw '*[🔥]Elaina AI: يوجد بعض المشاكل في معالجة هذا الأمر، يرجى التحلي بالصبر. سأقوم بإرسال طلبك قريبًا. *';
  }
};
handler.help = ['play', 'play2'].map((v) => v + ' < busqueda >');
handler.tags = ['downloader'];
handler.command = /^(شغل|شغل2)$/i;
